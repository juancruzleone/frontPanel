...
import { validateForm } from "../validators/deviceValidations";
...
const useInstallations = () => {
  ...
  const loadInstallationDetails = async (id: string) => {
    ...
    setError(null);
    try {
      const [installation, devices] = await Promise.all([
        fetchInstallationById(id),
        fetchInstallationDevices(id),
      ]);
  ...
  const editInstallation = async (id: string, updatedData: Installation): Promise<{ message: string }> => {
    try {
      ...
      setInstallations((prev) =>
        prev.map((inst) => (inst._id === id ? updatedInstallation : inst))
      );
