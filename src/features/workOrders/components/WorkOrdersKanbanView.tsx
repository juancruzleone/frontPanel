...
import { translateWorkOrderStatus, translatePriority } from '../../../shared/utils/backendTranslations'
import { Eye, Clock, MapPin, Play, Check, User, Edit, Trash } from 'lucide-react'
...
const KANBAN_STATUSES = ['pendiente', 'asignada', 'en_progreso', 'completada', 'cancelada']
      ...
      <div className={styles.kanbanCardFooter}>
        <div className={styles.kanbanActions}>
          <Tooltip content={t('workOrders.tooltips.viewDetails')}>
            <button className={styles.iconButton} onClick={() => onOpenDetails(order)}>
              <Eye size={16} />
            </button>
          </Tooltip>
          ...
          {order.estado === "asignada" && permissions?.canStartWorkOrder && (
            <Tooltip content={t('workOrders.startOrder')}>
              <button className={styles.iconButton} onClick={() => onStart(order._id!)}>
                <Play size={16} />
              </button>
            </Tooltip>
          )}
          ...
          {order.estado === "en_progreso" && permissions?.canCompleteWorkOrder && (
            <Tooltip content={t('workOrders.completeOrder')}>
              <button className={styles.iconButton} onClick={() => onOpenComplete(order)}>
                <Check size={16} />
              </button>
            </Tooltip>
          )}
          ...
          {permissions?.canAssignWorkOrders && ["pendiente", "asignada"].includes(order.estado) && (
            <Tooltip content={t('workOrders.assignTechnician')}>
              <button className={styles.iconButton} onClick={() => onOpenAssign(order)}>
                <User size={16} />
              </button>
            </Tooltip>
          )}
          ...
          {permissions?.canEditWorkOrders && ["pendiente", "asignada"].includes(order.estado) && (
            <Tooltip content={t('workOrders.editOrder')}>
              <button className={styles.iconButton} onClick={() => onOpenEdit(order)}>
                <Edit size={16} />
              </button>
            </Tooltip>
          )}
          ...
          {permissions?.canDeleteWorkOrders && (
            <Tooltip content={t('workOrders.deleteOrder')}>
              <button className={styles.iconButton} onClick={() => onOpenDelete(order)}>
                <Trash size={16} />
              </button>
            </Tooltip>
          )}
            ...
            >
              <div className={styles.kanbanColumnHeader}>
                <h3 className={styles.kanbanColumnTitle}>
                  ...
                  <span className={styles.kanbanCount}>{ordersInStatus.length}</span>
                </h3>
              </div>
              <div className={`${styles.kanbanCardsList} ${isDropTarget ? styles.kanbanDropTarget : ''}`}>
