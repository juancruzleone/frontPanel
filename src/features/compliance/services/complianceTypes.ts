...
export type CatalogState =
  ...
  | 'ERROR'
export type CatalogParameterType = 'number' | 'integer' | 'boolean' | 'string'
...
export interface CatalogParameterDefinition {
  ...
  min?: number
  max?: number
  allowed?: CatalogParameterValue[]
}
export interface CatalogControlReference {
  controlKey: string
  version: number
  scope: string
  parameterDefinitions: CatalogParameterDefinition[]
}
export interface CatalogEvaluatorDescriptor {
  evaluatorKey: string
  version: number
  operationId: string
  implementationVersion: string
}
export interface CatalogRights {
  author: 'Leonix'
  rightsStatus: 'original_operational_content'
}
...
export interface CatalogPackDetail {
  packKey: string
  version: number
  state: 'published'
  rights: CatalogRights
  controls: CatalogControlReference[]
  evaluators: CatalogEvaluatorDescriptor[]
}
export interface CatalogAssignment {
  ...
  controlScopes: Array<{ controlKey: string; version: number; scope: string }>
  createdAt?: string
  updatedAt?: string
}
export interface CatalogRunSummary {
  ...
  counts: Record<CatalogState, number>
  score: number | null
  createdAt?: string
  startedAt?: string
  finishedAt?: string
