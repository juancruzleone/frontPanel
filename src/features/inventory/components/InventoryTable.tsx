	...
	const columns = [
		...
		},
		{
			...
			render: (item: InventoryItem) => (
				<span
					className={
						item.currentStock <= item.minimumStock ? styles.lowStock : ""
					}
				>
		...
		},
		{
			...
			render: (item: InventoryItem) => {
				...
				return (
					<span
						className={`${styles.statusBadge} ${isLowStock ? styles.statusLow : styles.statusOk}`}
					>
						{isLowStock
		...
		},
		{
			...
			align: "center" as const,
			render: (item: InventoryItem) => {
				const hasInventoryItemId =
					item.inventorySource !== "asset" && Boolean(item._id);
