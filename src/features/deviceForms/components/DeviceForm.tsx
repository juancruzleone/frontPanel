...
const DeviceForm: React.FC = () => {
	...
	const [, setDatePickerField] = useState<string | null>(null);
	...
	const handleViewHistory = async () => {
		...
		const validCachedHistory =
			maintenanceOwnerId === userId ? historyByDevice[deviceId] : null;
	...
	if (loading)
		return <div className={styles.loader}>{t("deviceForm.loading")}</div>;
	if (!deviceInfo)
		return <div className={styles.error}>{t("deviceForm.notFound")}</div>;
				...
				{pendingSubmissions.length > 0 && (
					<div className={styles.pendingAlert}>
						<div className={styles.alertHeader}>
							<Clock size={18} />
							<h3>
								{t("deviceForm.pendingSubmissions", {
							...
							{pendingSubmissions.map((submission) => (
								<div key={submission.id} className={styles.pendingItem}>
									<span className={styles.pendingDate}>
										{t("deviceForm.savedAt", {
											date: formatTimestamp(submission.timestamp),
										})}
									</span>
									{submission.retryCount > 0 && (
										<span className={styles.retryBadge}>
											{t("deviceForm.retryCount", {
												count: submission.retryCount,
											})}
										</span>
									)}
								</div>
							...
							<div className={styles.cardContent}>
								<div className={styles.infoItem}>
									<span className={styles.infoLabel}>
										{t("deviceForm.company")}:
									</span>
									<span className={styles.infoValue}>
										{installationInfo.company}
									</span>
								</div>
								<div className={styles.infoItem}>
									<span className={styles.infoLabel}>
										{t("deviceForm.installationType")}:
									</span>
									<span className={styles.infoValue}>
				...
				>
					<div className={styles.formSection}>
						<h2 className={styles.sectionTitle}>
							{t("deviceForm.maintenanceDetails", "Detalles del Mantenimiento")}
						</h2>
					...
					<div className={styles.formSection}>
						<h2 className={styles.sectionTitle}>
							{t("deviceForm.evidenceSection", "Evidencia y Firma")}
						</h2>
						...
						<div className={styles.formGroup}>
							...
							<p className={styles.fieldDescription}>
								{t(
									"deviceForm.evidencePhotosDescription",
									"Agrega fotos que documenten el trabajo realizado",
								)}
							</p>
							<div className={styles.photoUploadContainer}>
								...
								>
									<Camera size={20} />
									<span>{t("deviceForm.takePhoto", "Tomar foto")}</span>
								</label>
								...
								>
									<Upload size={20} />
									<span>{t("deviceForm.uploadPhoto", "Cargar foto")}</span>
								</label>
							</div>
						...
						<div className={styles.formGroup}>
							...
							<p className={styles.fieldDescription}>
								{t(
									"deviceForm.signatureDescription",
						...
						>
							{submitting ? (
								...
								<>
									...
									<span>
										{isOnline
											? t("deviceForm.sendMaintenance")
											: t("deviceForm.saveMaintenance")}
									</span>
								</>
							)}
						</button>
