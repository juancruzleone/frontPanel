...
const Nav = () => {
  ...
  const [isWorkOrdersMenuOpen, setIsWorkOrdersMenuOpen] = useState(() => isWorkOrdersSectionActive)
  const [isWorkOrdersHovered, setIsWorkOrdersHovered] = useState(false)
  const [isOperationMenuOpen, setIsOperationMenuOpen] = useState(() => isOperationSectionActive)
  const [isOperationHovered, setIsOperationHovered] = useState(false)
  const [isMaintenanceMenuOpen, setIsMaintenanceMenuOpen] = useState(() => isMaintenanceSectionActive)
  ...
  }, [isWorkOrdersSectionActive, isOperationSectionActive, isMaintenanceSectionActive])
        ...
        <div className={styles.navContent}>
          ...
          <ul className={styles.menu}>
            {!isSuperAdminUser && !isClientUser && (
              <li>
                <NavLink to={homeRoute} className={({ isActive }) => (isActive ? styles.active : "")} onClick={() => setIsMenuOpen(false)}>
            ...
            {!isSuperAdminUser && !isClientUser && (
              ...
              >
                <button
                  ...
                  aria-expanded={isWorkOrdersMenuOpen || (isSidebarCollapsed && isWorkOrdersHovered)}
                >
                  <span className={styles.groupButtonContent}>
                    <Wrench size={20} /> <span className={styles.linkText}>{t('nav.workOrdersGroup')}</span>
                  </span>
                  <ChevronDown
                    size={16}
            ...
            {!isSuperAdminUser && (
              <li data-tour="nav-installations">
                <NavLink to={installationsRoute} className={({ isActive }) => (isActive ? styles.active : "")} onClick={() => setIsMenuOpen(false)}>
            ...
            {!isTechnicianUser && !isSuperAdminUser && !isClientUser && (
              <li data-tour="nav-assets">
                <NavLink to={assetsRoute} className={({ isActive }) => (isActive ? styles.active : "")} onClick={() => setIsMenuOpen(false)}>
            ...
            {!isSuperAdminUser && !isClientUser && (
              ...
              >
                <button
                  ...
                  aria-expanded={isOperationMenuOpen || (isSidebarCollapsed && isOperationHovered)}
                >
                  <span className={styles.groupButtonContent}>
                    <Briefcase size={20} /> <span className={styles.linkText}>{t('nav.operation')}</span>
                  </span>
                  <ChevronDown
                    size={16}
                ...
                >
                  <NavLink to={inventoryRoute} className={({ isActive }) => `${styles.submenuLink} ${isActive ? styles.active : ""}`} onClick={() => { setIsMenuOpen(false); setIsOperationHovered(false) }}>
                    <Boxes size={20} /> <span className={styles.linkText}>{t('nav.inventory')}</span>
                  </NavLink>
                  {!isTechnicianUser && !isSuperAdminUser && !isClientUser && (
                    <NavLink to={personalRoute} className={({ isActive }) => `${styles.submenuLink} ${isActive ? styles.active : ""}`} onClick={() => { setIsMenuOpen(false); setIsOperationHovered(false) }}>
                      <User size={20} /> <span className={styles.linkText}>{t('nav.personal')}</span>
                    </NavLink>
                  )}
                  {isAdminUser && (
                    <NavLink to={suppliersRoute} className={({ isActive }) => `${styles.submenuLink} ${isActive ? styles.active : ""}`} onClick={() => { setIsMenuOpen(false); setIsOperationHovered(false) }}>
            ...
            {(isAdminUser || isSuperAdminUser) && (
              <li>
                <NavLink to={auditRoute} className={({ isActive }) => (isActive ? styles.active : "")} onClick={() => setIsMenuOpen(false)}>
